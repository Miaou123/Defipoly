...
import { X, ShoppingCart, Shield, TrendingDown, TrendingUp, Zap, Clock, Target, AlertTriangle, Star, DollarSign } from 'lucide-react';
...
export function MobileHelpModal({ isOpen, onClose, type }: MobileHelpModalProps) {
  ...
  const getIcon = () => {
    switch (type) {
      case 'buy': return <ShoppingCart className={`w-6 h-6 ${colors.icon}`} />;
  ...
  return (
    ...
    >
      <div 
        className={`bg-black/95 backdrop-blur-xl rounded-xl border ${colors.border} max-w-[320px] w-full overflow-hidden`}
