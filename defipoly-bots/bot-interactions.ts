...
// Updated paths for standalone bot folder
...
// Wallet addresses (matching frontend constants)
const MARKETING_WALLET = new PublicKey("FoPKSQ5HDSVyZgaQobX64YEBVQ2iiKMZp8VHWtd6jLQE");
...
async function buyProperties(walletIds: number[], propertyId: number, slots: number = 1) {
  ...
  for (const id of walletIds) {
    ...
    try {
      ...
      const marketingTokenAccount = getAssociatedTokenAddress(TOKEN_MINT, MARKETING_WALLET);
      ...
      await program.methods
        ...
        .accountsPartial({
          ...
          set_cooldown: setCooldownPDA,
          set_ownership: setOwnershipPDA,
          set_stats: setStatsPDA,
          player_account: playerPDA,
          player_token_account: playerTokenAccount,
          reward_pool_vault: rewardPoolVault,
...
async function claimRewards(walletIds: number[]) {
  ...
  for (const id of walletIds) {
    ...
    try {
      ...
      // Get all properties (0-21) for remaining accounts - match frontend structure
      const ownershipAccounts = [];
      const propertyAccounts = [];
      ...
      for (let propId = 0; propId < 22; propId++) {
        ...
        try {
          ...
          if (ownership.slotsOwned > 0) {
            const [propertyPDA] = getPropertyPDA(propId);
